...
export const withSidebarPlugin = createHigherOrderComponent(
	...
	(WrappedComponent) => (ownProps) => {
		...
		const titleElement = isFunction(title)
			? title(isFunction(titleProps) ? titleProps() : titleProps)
			: title
		...
		if (slot === 'setting')
			return (
				<PluginDocumentSettingPanel name={name} title={titleElement} className={className}>
					<WrappedComponent forceUpdate={forceUpdate} {...ownProps} />
				</PluginDocumentSettingPanel>
			)
		...
		if (slot === 'sidebar')
			return (
				<>
					{moreItem && (
						<PluginSidebarMoreMenuItem target={name} icon={moreIcon}>
		...
		return (
			<Fill name={slot} className={className}>
...
export const withPlugin = (pluginProps) =>
	compose([
		// get Plugin props
...
export const withPluginMeta = (pluginProps) =>
	compose([
		// get plugin props
		createHigherOrderComponent(
			// eslint-disable-next-line react/display-name

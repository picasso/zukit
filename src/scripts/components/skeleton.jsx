...
import { compareVersions } from '../utils.jsx'
import ZukitSidebar from './sidebar.jsx'
...
function editorClasses(element, more = '', wp = null) {
	...
	if (isEmpty(element)) {
		...
		const verClass =
			`wp_${ver}${ver !== v2digs ? ' ' + v2digs : ''}` +
			(compareVersions(wp, '5.4') < 0 ? ' wp_less_5_4' : '')
...
const ZukitSkeleton = ({
	...
	const [loading, setLoading] = useState(
		...
		reduce(
			actions,
			(acc, val) => ((acc[val.value] = false), acc),
	...
	useEffect(() => {
		const infoKeys = reduce(
			moreInfo,
			(acc, value) => {
				forEach(castArray(get(value, 'depends', [])), (val) =>
					includes(acc, val) ? null : acc.push(val),
				)
				return acc
			},
			[],
	...
	const pluginIcon = !info.icon ? null : (
		<BlockIcon icon={<RawHTML>{info.icon}</RawHTML>} showColors />
	)
		...
		<div className={editorClasses(null, cprefix, wp)}>
			<div className={editorClasses('body')}>
				<SlotFillProvider>
					...
					>
						<div className="components-editor-notices__dismissible">{noticeUI}</div>
						<div
...
export default compose([withNotices])(ZukitSkeleton)

...
const ZukitTable = ({
	...
	const { align: cellAlign = [], style: cellStyle = [], className: cellClass = [] } = config || {}
	...
	const withContent = (rowIndex, content, params) => {
		if (isPlainObject(content)) {
			...
			const icon = <BlockIcon icon={svg ? <RawHTML>{svg}</RawHTML> : dashicon} showColors />
			return !tooltip ? (
				icon
			) : (
			...
			if (dynamicValue !== undefined) {
				...
				return asMarkdown
					? simpleMarkdown(dynamicValue, { br: true, json: true })
					: dynamicValue
			}
			return content
		}
					...
					{map(head, ({ content, align, style }, cellIndex) => (
						<div
							className={mergeClasses(
								'cell',
								'head',
								withClass(null, cellIndex, align),
							)}
							key={cellIndex}
						...
						>
							{map(cells, ({ content, align, style, params }, cellIndex) => (
								<div
									className={mergeClasses(
										'cell',
										withClass(rowIndex, cellIndex, align, params),
									)}
									key={cellIndex}
									aria-label=""
									style={withStyle(

(function () {
  ...
  */
  ...
  var AdvTextControl = function AdvTextControl(_ref) {
    ...
    }, [isPassword, visible, onChange]); // Debounce ---------------------------------------------------------------]
    // using temporaryName while debouncing name changes

    var _useState3 = useState$2(value),
        _useState4 = _slicedToArray(_useState3, 2),
        temporaryValue = _useState4[0],
        setTemporaryValue = _useState4[1]; // using debouncing to reduce the number of calls to the onChange handler
    // eslint-disable-next-line react-hooks/exhaustive-deps


    var onUpdateValue = useCallback$5(debounce(function (value) {
      onChange(value);
    }, debounceDelay), [onChange, debounceDelay]);
    var onChangeValue = useCallback$5(function (value) {
      setTemporaryValue(value);
      onUpdateValue(value);
    }, [onUpdateValue]); // Validate ---------------------------------------------------------------]
  ...
  function defaultGetter(state, stateKey, valueKey) {
    return get$4(state, [stateKey, valueKey]);
  }
  ...
  var useCoreDataGeneric = function useCoreDataGeneric(key, params) {
    ...
    return isEmpty$4(data) ? null : data;
  ...
  var useFolders = function useFolders() {
    ...
    return isEmpty$4(folders) ? null : folderId === null ? asOptions ? folderOptions(folders) : folders : get$5(folders, folderId, null);
  ...
  var useGalleries = function useGalleries() {
    ...
    return isEmpty$4(galleries) ? null : postId === null ? galleries : get$5(galleries, postId, null);
  }; // Get Loaders ----------------------------------------------------------------]
  ...
  var withLoaders = createHigherOrderComponent$1(withSelect(function (select, _ref) {
    ...
    return {
      loaderHTML: isEmpty$4(loaderRaw) ? null : loaderRaw
    };
  ...
  var useLoaders = function useLoaders() {
    ...
    return isEmpty$4(loaders) ? null : index === null ? loaders : get$5(loaders, index, null);
  };
  ...
  function setupOptionsStore(router) {
    ...
    var useGetOptions = function useGetOptions(keys) {
      ...
      return isEmpty$5(gotOptions) ? null : gotOptions;
  ...
  function logLevel() {
    ...
    if (newLevel) {
      if (_$2.includes(['short', 1], newLevel)) config.level = 1;else if (_$2.includes(['default', 'normal', 2], newLevel)) config.level = 2;else if (_$2.includes(['verbose', 'full', 3], newLevel)) config.level = 3;else if (_$2.includes(['none', 0], newLevel)) config.level = 0;
    }
  ...
  function color_by(message) {
    ...
    if (config.colors.use) return dcolors.use; // remove any var names which may lead to false-positive test
  ...
  function maybeForce(message) {
    return _$2.endsWith(message, '!') || _$2.endsWith(message, '?');
  }
  /* eslint-disable no-console */


  function logMaybeNode(node) {
    if (_$2.isFunction(node)) return;
  ...
  function logWithColors(messages, messageColors) {
    ...
    config.colors = _$2.mapValues(config.colors, function () {
      return false;
    });
    config.mods = _$2.mapValues(config.mods, function () {
      return false;
    });
  }
  ...
  function log(message) {
    ...
    if (message) {
      ...
      var colors = [color_by(message), dcolors.name, null];
  ...
  function logGroup(obj) {
    ...
    if (groupName && _$2.isPlainObject(obj)) {
      console.groupCollapsed('%c%s', "font-weight: bold; color: ".concat(dcolors.name, "; padding: 3px;"), groupName.trim());
      closeMore = true;
    }

    for (var key in obj) {
      if (withoutNil && _$2.isNil(obj[key])) continue; // › »

      var keyName = groupName && _$2.isArray(obj) ? "".concat(groupName, "[").concat(key, "]") : key;

      if (_$2.isFunction(obj[key])) {
        console.dir(obj);
        break;
  ...
  function warn(message, data, more) {
    ...
    if (!_$2.isUndefined(data) && canIlog(message, true)) logMaybeNode(data);
    if (!_$2.isUndefined(more) && canIlog(message, true)) logMaybeNode(more);
  ...
  function error(message, data) {
    ...
    if (_$2.isUndefined(data)) console.error(message);else {
  ...
  function simplify(name, value) {
    ...
    if (_$2.isArray(value) && value.length === 1) {
      var simpleValue = ["".concat(name, " \u21E2 ").concat(name, "[0]"), value[0]];
    ...
    if (root && _$2.isObjectLike(value) && !_$2.isEmpty(value) && _$2.every(value, _$2.isArray)) {
      return [name, _$2.reduce(value, function (result, v, k) {
        var _simplify = simplify(k, v, false),
            _simplify2 = _slicedToArray(_simplify, 2),
            key = _simplify2[0],
            val = _simplify2[1];
    ...
    if (root && _$2.isPlainObject(value) && _$2.keys(value).length === 1) {
      var _$keys = _$2.keys(value),
          _$keys2 = _slicedToArray(_$keys, 1),
          keyName = _$keys2[0];

      var _simpleValue = ["".concat(name, ".").concat(keyName), value[keyName]];
      return [name, _simpleValue];
    }
  ...
  function cloneValue(value) {
    ...
    if (_$2.isNil(value)) return value; // first try with lodash 'cloneDeepWith'

    var nodeCloner = function nodeCloner(value) {
      return _$2.isElement(value) ? value.cloneNode(true) : undefined;
    };

    var cloned = _$2.cloneDeepWith(value, nodeCloner);

    if (!_$2.isEmpty(cloned)) return cloned; // try with JSON if 'cloneDeepWith' failed
    ...
    var circularReplacer = function circularReplacer(_key, value) {
      ...
      return _$2.isUndefined(value) ? '__undefined' : value;
    };
  ...
  function renderComponent() {
    ...
    log.apply(void 0, ["".concat(componentName('renderComponent'), " [render]")].concat(data));
  }
  ...
  function renderComponentWithId(clientId) {
    ...
    log.apply(void 0, ["".concat(componentName('renderComponentWithId'), " [").concat(shortenId({
      clientId: clientId
    }), "]")].concat(data));
  }

  function useInComponent() {
    var _funcAndComponentName = funcAndComponentNames('useInComponent'),
        _funcAndComponentName2 = _slicedToArray(_funcAndComponentName, 2),
        componentAndVar = _funcAndComponentName2[0],
        func = _funcAndComponentName2[1];

    var _combineNames = combineNames(componentAndVar, true),
        _combineNames2 = _slicedToArray(_combineNames, 2),
        component = _combineNames2[0],
        funcVar = _combineNames2[1];

    var funcName = func !== 'useMemo' ? 'useCallback' : func;
    var key = funcVar ? "".concat(funcName, " : ").concat(funcVar) : funcName;
    config.colors.same = true;
    config.colors.use = true;

    for (var _len5 = arguments.length, data = new Array(_len5), _key6 = 0; _key6 < _len5; _key6++) {
      data[_key6] = arguments[_key6];
    }

    log.apply(void 0, ["".concat(component, " [").concat(key, "]")].concat(data));
  }

  function dataInComponent(data) {
    var marker = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    var className = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : '_';
    var previous = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : '';
    var component = componentName(_$2.union(['dataInComponent'], _$2.split(previous, ', ')), className);

    var _$keys3 = _$2.keys(data),
        _$keys4 = _toArray(_$keys3),
        firstKey = _$keys4[0],
        rest = _$keys4.slice(1);

    var isSingleKey = rest.length === 0;
    var key = isSingleKey ? firstKey : _$2.join([firstKey].concat(_toConsumableArray(rest)), ', ');
    var value = isSingleKey ? data[firstKey] : data; // simplify only for single key

    if (isSingleKey) {
      var _simplify3 = simplify(key, value);

      var _simplify4 = _slicedToArray(_simplify3, 2);

      key = _simplify4[0];
      value = _simplify4[1];
    }

    var name = marker ? "".concat(key, " : ").concat(String(marker)) : key;
    config.mods.forseNil = true;
    config.mods.consoleDir = true;
    config.colors.data = true;
    if (_$2.isArray(value)) log.apply(void 0, ["".concat(component, " [").concat(name, "]")].concat(_toConsumableArray(cloneValue(value))));else log("".concat(component, " [").concat(name, "]"), cloneValue(value));
  }

  function infoInComponent(message) {
    var infoNames = componentName('infoInComponent');
    config.mods.consoleDir = true;
    config.colors.info = true;

    for (var _len6 = arguments.length, data = new Array(_len6 > 1 ? _len6 - 1 : 0), _key7 = 1; _key7 < _len6; _key7++) {
      data[_key7 - 1] = arguments[_key7];
    }

    log.apply(void 0, ["".concat(message, " [").concat(combineNames(infoNames), "]")].concat(data));
  }

  function infoInComponentWithId(clientId, message) {
    var infoNames = componentName('infoInComponentWithId');
    config.mods.consoleDir = true;
    config.colors.info = true;

    for (var _len7 = arguments.length, data = new Array(_len7 > 2 ? _len7 - 2 : 0), _key8 = 2; _key8 < _len7; _key8++) {
      data[_key8 - 2] = arguments[_key8];
    }

    log.apply(void 0, ["".concat(message, " with ").concat(shortenId({
      clientId: clientId
    }), " [").concat(combineNames(infoNames), "]")].concat(data));
  ...
  function logRequestResponse(type, route, options, response) {
    ...
    var message = _$2.get(messages, type) || "? Ajax ".concat(type);
    var logData = response ? response : options;

    if (response) {
      logData = _$2.merge(logData, {
        timestamp: new Date().toString()
      });

      if (_$2.isEmpty(response)) {
        message += " : response is empty ";
      }
    } // Log request URL and its data (options or response) if presented


    if (_$2.isEmpty(logData)) {
      log(message);
    } else {
      log(">".concat(message));
      logGroup(logData);
  ...
  function propsAndState(name, compId) {
    ...
    if (props || state) {
      tracedComps["".concat(name, "-").concat(compId)] = [props, state];
    } else {
      return tracedComps["".concat(name, "-").concat(compId)] || [{}, {}];
    }
  ...
  function useTraceUpdate(props) {
    ...
    var key = combineNames(componentName(trackClientId ? 'useTraceUpdate,useTraceUpdateWithId' : 'useTraceUpdate'));
    var id = trackClientId ? " with ".concat(shortenId(props)) : '';
    var compId = shortenId(props, true);

    var _propsAndState = propsAndState(key, compId),
        _propsAndState2 = _slicedToArray(_propsAndState, 2),
        prev = _propsAndState2[0],
        prevState = _propsAndState2[1]; // useEffect(() => {
    ...
    var changedProps = Object.entries(props).reduce(function (ps, _ref) {
      ...
      if (prev[k] !== v) {
        ps[0][k] = v;
        ps[1]["".concat(k)] = prev[k]; // _
      }

      return ps;
    }, [{}, {}]);
    var changedState = Object.entries(state).reduce(function (ss, _ref3) {
      ...
      if (prevState[k] !== v) {
        ss[0][k] = v;
        ss[1]["".concat(k)] = prevState[k]; // _
      }

      return ss;
    }, [{}, {}]);
    var hasProps = Object.keys(changedProps[0]).length > 0;
    var hasState = Object.keys(changedState[0]).length > 0;
    config.mods.consoleDir = true;
    config.colors.trace = hasProps || hasState;

    if (hasProps) {
      changedProps = _$2.reduce(changedProps, function (props, p, index) {
        var _simplify5 = simplify(index ? 'prevProps' : 'props', p),
            _simplify6 = _slicedToArray(_simplify5, 2),
            simplified = _simplify6[1];

        if (isIterable(simplified)) props.push.apply(props, _toConsumableArray(simplified));else props.push(simplified);
        return props;
      ...
      if (changedProps.length === 4 && changedProps[0] === 'props.attributes') {
        ...
        _$2.forEach(changedProps[1], function (_val, key) {
          if (changedProps[1][key] !== changedProps[3][key]) {
            next[key] = changedProps[1][key];
            _prev[key] = changedProps[3][key];
          }
        });
        ...
        changedProps[1] = next;
        changedProps[2] += '*';
        changedProps[3] = _prev;
      }
    }

    if (hasState) {
      changedState = _$2.reduce(changedState, function (state, s, index) {
        var _simplify7 = simplify(index ? 'prevState' : 'state', s),
            _simplify8 = _slicedToArray(_simplify7, 2),
            simplified = _simplify8[1];

        if (isIterable(simplified)) state.push.apply(state, _toConsumableArray(simplified));else state.push(simplified);
        return state;
      }, []);
    }

    if (hasProps && !hasState) log.apply(void 0, ["Traced changes".concat(id, " [").concat(key, " : props]")].concat(_toConsumableArray(changedProps)));
    if (!hasProps && hasState) log.apply(void 0, ["Traced changes".concat(id, " [").concat(key, " : state]")].concat(_toConsumableArray(changedState)));
    if (hasProps && hasState) log.apply(void 0, ["Traced changes".concat(id, " [").concat(key, " : props & state]")].concat(_toConsumableArray(changedProps), _toConsumableArray(changedState)));
  ...
  function useAkaMount() {
    var name = combineNames(componentName('useAkaMount'));
    useEffect$2(function () {
      config.mods.consoleDir = true;
      config.colors.info = true;
      log("#aka componentDidMount [".concat(name, "]"));
      return function () {
        config.mods.consoleDir = true;
        config.colors.info = true;
        log("#aka componentWillUnmount [".concat(name, "]"));
  ...
  function skipNames(name, previous) {
    var previousNames = _$2.isArray(previous) ? previous : _$2.split(previous, ',');
    return _$2.union([name], previousNames);
  }

  function combineNames(names) {
    var asArray = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;

    var _$split = _$2.split(names, '/'),
        _$split2 = _slicedToArray(_$split, 2),
        component = _$split2[0],
        _$split2$ = _$split2[1],
        funcVar = _$split2$ === void 0 ? false : _$split2$;

    return asArray ? [component, funcVar] : funcVar ? "".concat(component, " : ").concat(funcVar) : component;
  }

  function componentName() {
    var previous = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
    var componentAlt = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
    var stack = findOnStack(skipNames('componentName', previous), false);
    var name = _$2.isUndefined(stack[0]) ? '?' : stack[0].replace(/[<|/]+$/g, '');
    var altName = _$2.isUndefined(stack[1]) ? false : stack[1].replace(/[<|/]+$/g, ''); // component name should start with UpperCase

    if (name[0] === name[0].toUpperCase()) return name; // maybe we have something similar to component name?

    if (_$2.isString(altName) && altName[0] === altName[0].toUpperCase() && altName.length > 2) componentAlt = altName;
    return componentAlt ? "".concat(componentAlt, ".").concat(name, "()") : "".concat(name, "()");
  }

  function funcAndComponentNames() {
    var previous = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
    var stack = findOnStack(skipNames('funcAndComponentNames', previous), false);
    return [_$2.isUndefined(stack[0]) ? '?' : stack[0].replace(/[<|/]+$/g, ''), _$2.isUndefined(stack[1]) ? '?' : stack[1].replace(/[<|/]+$/g, '')];
  }

  function findOnStack(previousNames) {
    var andJoin = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
    var removeFrames = skipNames('findOnStack', previousNames);
    var stack = stackParser(new Error()).slice(0, 10); // we need only the first few lines

    stack = _$2.filter(stack, function (frame) {
      return _$2.findIndex(removeFrames, function (removed) {
        var re = new RegExp('^' + removed + '[\\d|\\W]*$', 'i');
        return re.test(frame.functionName);
      }) === -1;
    });
    stack = _$2.map(stack, function (frame) {
      return andJoin ? frame.source : frame.functionName;
    });
    return andJoin ? _$2.join(stack, '\n') : stack;
  }
  ...
  var debug = {
    ...
    cdata: function cdata(data, className) {
      dataInComponent(data, false, className, 'cdata');
    },
    // for data inside class components

(function () {
  ...
  */
  ...
  function dataInComponent(data) {
    ...
    var key = isSingleKey ? _.first(keys) : _.join(keys, "".concat(_a, ", ").concat(_a));
    var value = isSingleKey ? data[key] : data;
    var altName = marker ? ":".concat(_c).concat(String(marker)).concat(_c) : '';
  ...
  function infoInComponent(messageData) {
    ...
    var info = "".concat(_b).concat(component).concat(_b).concat(id, " ").concat(arrowSymbol, " ").concat(message);
  ...
  function parseWithColors(message, colors) {
    ...
    _.forEach(message, function (char) {
      if (char === _a) {
        ...
        format += "".concat(token).concat(_p[0], "%c");
        items.push(params);
        token = '';
      } else if (char === _p[1]) {
        format += "".concat(token, "%c");
  ...
  function logSimplified(val) {
    {
      ...
      if (keys.length === 1) {
        ...
        var message = ["value ".concat(kind, " ").concat(_p[0]).concat(firstKey).concat(_p[1])];
  ...
  function logAddedRemoved(added, removed) {
    ...
    if (addedKeys) {
      ...
      message += "added ".concat(_b).concat(addedKeys).concat(_b, " ").concat(_p[0]).concat(_.join(keys, ', ')).concat(_p[1]).concat(removedKeys ? ', ' : '');
    }

    if (removedKeys) {
      var _keys = removed.length > compactKeysCount ? _.concat(_.take(removed, compactKeysCount), ['and more...']) : removed;

      message += "removed ".concat(_b).concat(removedKeys).concat(_b, " ").concat(_p[0]).concat(_.join(_keys, ', ')).concat(_p[1]);
    }
  ...
  function logWasNow(was, now, keys) {
    ...
    if (changed && changed.length === 1) {
      ...
      var message = "".concat(chevronSymbol, "changed for ").concat(_b, "key").concat(_b, " ").concat(_p[0]).concat(firstСhanged).concat(_p[1]);
      ...
      logSmart(wasValue);
      logAsOneString(changed ? "".concat(_c, "now").concat(_c, " changed for ").concat(_b, "keys").concat(_b, " ").concat(_p[0]).concat(_.join(changed, ', ')).concat(_p[1]) : "".concat(_c, "now").concat(_c));
      logSmart(nowValue);
  ...
  function logChanges(keys, prevValues, values) {
    ...
    _.forEach(updated, function (key) {
      ...
      var message = "".concat(chevronSymbol).concat(_a).concat(key).concat(_a);
      if (isSimpleType(value)) logAsOneString(message, prevValues[key], arrowSymbol, value);else {
        if (_.isFunction(value)) {
          ...
          if (!changed.length && !(addedKeys !== null && addedKeys !== void 0 && addedKeys.length) && !(removedKeys !== null && removedKeys !== void 0 && removedKeys.length)) {
            ...
            var keyMsg = "".concat(message, " @1@ ").concat(_b, "@2@").concat(_b, " ").concat(_p[0]).concat(_.join(changed, ', ')).concat(_p[1]);
  ...
  function componentName() {
    ...
    if (name[0] === name[0].toUpperCase()) return name.replace(/_/g, '⎽'); // maybe we have function?
    // replace simple underscore with "other" underscore to avoid problems with 'colored' console
    // (underscore is used for _bold text_)

    var func = name.replace('/zu_blocks', '').replace(/_/g, '⎽').replace(/[/]/g, '.');
    return "".concat(func, "()");
  }

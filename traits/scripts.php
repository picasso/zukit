...
trait zukit_Scripts {
	...
	final protected function singleton_config_scripts() {
	...
	public function is_origin($get_root = false) {
		$root_dirname = dirname($this->root()); // self::$zukit_root
		return $get_root ? $root_dirname : $root_dirname === ($this->dir . '/zukit');
	}

	public function zukit_dirname($subdir = null) { // self::$zukit_root
		return dirname($this->root()) . (empty($subdir) ? '' : '/' . ltrim($subdir, '/'));
	}
		...
		if (substr($file, 0, 4) === 'http') {
			...
			if ($is_absolute) {
				...
				if (substr($file, 0, 5) === 'zukit') {
					...
					// self::$zukit_root
					$src = plugin_dir_url($this->root()) . str_replace(plugin_dir_path($this->root()), '', $filepath);
				}
			} else {
